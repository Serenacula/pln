...
- **Name semantics** — PLN assigns no meaning to panel names; what a name refers to is entirely implementation-defined. In particular, implementations with fixed UI regions may define a **reserved vocabulary** of names that map onto those regions — for example, an editor plugin might recognise `sidebar`, `editor`, and `panel` as its own sidebar, editor area, and bottom panel, and reject or ignore names outside that vocabulary. This pattern is intentional and encouraged where arbitrary panels cannot be created. Implementations defining a reserved vocabulary should document it, and should say how unrecognised names are handled.
